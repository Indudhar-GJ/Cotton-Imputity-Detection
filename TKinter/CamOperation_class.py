...
class CameraOperation:
    ...
    def Set_parameter(self):
        ...
        if self.b_open_device:
            ret = self.obj_cam.MV_CC_SetEnumValue("ExposureAuto", row[7])
            if ret != 0:
                print('show error', 'set ExposureAuto fail! ret = ' + To_hex_str(ret))
                return ret
            time.sleep(0.2)

            ret = self.obj_cam.MV_CC_SetBoolValue("AcquisitionLineRateEnable", bool(row[1]))
            if ret != 0:
                print('show error', 'set AcquisitionLineRateEnable fail! ret = ' + To_hex_str(ret))
                return ret    

            ret = self.obj_cam.MV_CC_SetIntValue("AcquisitionLineRate", row[8])
            if ret != 0:
                print('show error', 'set acquistion line rate fail! ret = ' + To_hex_str(ret))
                return ret
            
            ret = self.obj_cam.MV_CC_SetIntValue("AutoExposureTimeLowerLimit", row[9])
            if ret != 0:
                print('show error', 'set AutoExposureTimeLowerLimit fail! ret = ' + To_hex_str(ret))
                return ret
            
            ret = self.obj_cam.MV_CC_SetIntValue("AutoExposureTimeUpperLimit", row[10])
            if ret != 0:
                print('show error', 'set AutoExposureTimeUpperLimit fail! ret = ' + To_hex_str(ret))
                return ret
            
            ret = self.obj_cam.MV_CC_SetIntValue("Width", row[11])
            if ret != 0:
                print('show error', 'set Width fail! ret = ' + To_hex_str(ret))
                return ret
            
            ret = self.obj_cam.MV_CC_SetIntValue("Height", row[12])
            if ret != 0:
                print('show error', 'set Height fail! ret = ' + To_hex_str(ret))
                return ret
            
            ret = self.obj_cam.MV_CC_SetBoolValue("ReverseX", bool(row[2]))
            if ret != 0:
                print('show error', 'set ReverseX fail! ret = ' + To_hex_str(ret))
                return ret
            
            ret = self.obj_cam.MV_CC_SetEnumValue("BalanceWhiteAuto", row[13])
            if ret != 0:
                print('show error', 'set BalanceWhiteAuto fail! ret = ' + To_hex_str(ret))
                return ret
                
            ret = self.obj_cam.MV_CC_SetEnumValue("PixelFormat", 0x01080009 ) #row[6] -- 0x01080009
            if ret != 0:
                print('show error', 'set PixelFormat fail! ret = ' + To_hex_str(ret))
                return ret
            
            ret = self.obj_cam.MV_CC_SetBoolValue("GammaEnable", bool(row[3]))
            if ret != 0:
                print('show error', 'set GammaEnable fail! ret = ' + To_hex_str(ret))
                return ret
            
            ret = self.obj_cam.MV_CC_SetEnumValue("GammaSelector", row[14])
            if ret != 0:
                print('show error', 'set GammaSelector fail! ret = ' + To_hex_str(ret))
                return ret
            
            ret = self.obj_cam.MV_CC_SetBoolValue("HueEnable", bool(row[4]))
            if ret != 0:
                print('show error', 'set HueEnable fail! ret = ' + To_hex_str(ret))
                return ret
            
            ret = self.obj_cam.MV_CC_SetBoolValue("SaturationEnable", bool(row[5]))
            if ret != 0:
    ...
    def Save_Bmp(self):
        ...
        file_path = "/home/cotseeds/Desktop/Cotseeds_Final/Code/images/"+ str(self.st_frame_info.nFrameNum) + ".bmp"
