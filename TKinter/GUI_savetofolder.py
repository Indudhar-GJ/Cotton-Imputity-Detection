...
# with open('inspection_config.json', 'r') as f:
#     config = json.load(f)
...
class ImageDetector:
    ...
    def process_tiles(self, tiles):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT Cloth, Thread, Jute, Packet FROM params WHERE id = 1")
        row = cursor.fetchone() 
        Cloth_conf, Thread_conf, Jute_conf, Packet_conf = row
        conn.close()
        ...
        for i, result in enumerate(results):
            ...
            for box in result.boxes:
                ...
                if Cloth_conf >= self.confidence_threshold and class_score == 0:
                    box_data = [int(x_min), int(y_min), int(x_max), int(y_max)]
                    tile_results.append((box_data, confidence_score, int(class_score)))
                    # thread_count += 1

                if Thread_conf >= self.confidence_threshold and class_score == 3:
                    box_data = [int(x_min), int(y_min), int(x_max), int(y_max)]
                    tile_results.append((box_data, confidence_score, int(class_score)))

                if Jute_conf >= self.confidence_threshold and class_score == 4:
                    box_data = [int(x_min), int(y_min), int(x_max), int(y_max)]
                    tile_results.append((box_data, confidence_score, int(class_score)))

                if Packet_conf >= self.confidence_threshold and class_score == 5:
                    box_data = [int(x_min), int(y_min), int(x_max), int(y_max)]
                    tile_results.append((box_data, confidence_score, int(class_score)))
...
if __name__ == "__main__":
    ...
    def trigger_once():
        ...
        folder_path = "images"
        image_extension = "*.bmp"
        detected_folder = "detected_tiles"
        clean_folder = "clean_tiles"
            ...
            while True:
                ...
                if not any(os.scandir('images')):
                    ...
                    try:
                        ...
                        for idx, tile_image in enumerate(tiles):
                            tile_path = os.path.join(detected_folder if (idx + 1) in detected_tiles else clean_folder,f"tile_{idx + 1}.png")
                            cv2.imwrite(tile_path, tile_image)  # Save image
    ...
    conn = sqlite3.connect("config1.db") 
    cursor = conn.cursor()
    cursor.execute("SELECT ModelPath FROM params WHERE id = 1")
    row = cursor.fetchone()
    model_path,=row
    conn.close()
    ...
    #NEW YOLOV12 MODEL -- 20 MARCH 2025
    detector = ImageDetector(model_path, 512)
