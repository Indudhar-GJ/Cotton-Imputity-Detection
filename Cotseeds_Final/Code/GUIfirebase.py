...
# Initialize Firebase
cred = credentials.Certificate("F:\internship work\DummyDataCreator\DummyDataCreator\cottonsense.json")  # Replace with actual path
firebase_admin.initialize_app(cred)
db = firestore.client()

def upload_detection_results(batch_id, detections):
    """
    Uploads detection results to Firebase under the given batch ID.
    """
    batch_ref = db.collection("batches").document(batch_id)
    
    batch_ref.set({
        "batchId": batch_id,
        "startDateAndTime": datetime.datetime.now().isoformat(),
        "endDateAndTime": "",
        "cottonWeight": 0,
        "totalImages": len(detections),
        "detections": detections,
        "images": []
    }, merge=True)
    print(f"Uploaded detection results for batch {batch_id}")


def process_and_upload(image_path, detector, batch_id):
    """Processes an image, detects objects, and uploads the results to Firebase."""
    tiles = split_image_fast(image_path, 512)
    preprocessed_tiles = [detector.preprocess_tile(tile) for tile in tiles]
    batch_results = detector.process_tiles(preprocessed_tiles)
    detected_tiles = {str(idx + 1): results for idx, results in enumerate(batch_results) if results}
    
    upload_detection_results(batch_id, detected_tiles)
    print(f"Processed and uploaded results for {image_path}")

# Auto-upload data every 5 minutes
def periodic_upload():
    while True:
        time.sleep(300)  # 5 minutes
        batch_id = str(uuid.uuid4())
        upload_detection_results(batch_id, {})

timer_thread = threading.Thread(target=periodic_upload, daemon=True)
timer_thread.start()
...
class ImageDetector:
    ...
    def process_tiles(self, tiles):
        ...
        try:
            for i, result in enumerate(results):
                ...
                for box in result.boxes:
                    ...
                    if confidence_score >= self.confidence_threshold:
                        ...
                        #thread_count += results.boxes.shape[0]
                        thread_count+=1
            ...
            with open("results_count.txt", "a") as file:
...
if __name__ == "__main__":
    ...
    #detector = ImageDetector("/home/ar/Desktop/Cotseeds_Final/Code/yolov8-seg-thread-detection3-int8.engine", 512)
    #bounding box model
    #detector = ImageDetector("/home/ar/Desktop/Cotseeds_Final/Code/yolov8s-boundingbox-threads1-int8.engine", 512)
    #new model
    detector = ImageDetector("/home/ar/Desktop/Cotseeds_Final/Code/yolov8-seg2-thread-detection3-fp16.engine", 512)
    #detector = ImageDetector("/home/ar/Desktop/Cotseeds_Final/Code/yolov8-seg2-thread-detection3-int8.engine", 512)
