...
from UI_Video_feed import Ui_MainWindow
import time
...
# Firebase setup
cred = credentials.Certificate("/home/cotseeds/Desktop/Cotseeds_Final/Code/cottonsense.json")
firebase_admin.initialize_app(cred, {
    'databaseURL': 'https://cotton-sense-default-rtdb.firebaseio.com/'
})

# Define PlantID (you can change this as needed)
plant_id = "COTSEEDSHBL"  # Replace with actual Plant ID

# Generate batch ID
batch_id = f"{int(time.time())}"

# Reference to the batch inside the specific Plant
batch_ref = db.reference(f'Plants/{plant_id}/active_batch/{batch_id}')

# Initialize batch entry inside the plant's active batch
batch_ref.set({
    "batchId": batch_id,
    "startDateAndTime": datetime.now().isoformat(),
    "endDateAndTime": "",
    "cottonWeight": 0,
    "totalImages": 0,
    "detections": {},
...
impurity_dict = {}
impurity_classes = ['cloth', 'conveyor belt', 'cotton', 'jute', 'packet', 'thread', 'wood']

for impurity in impurity_classes:
    impurity_dict[impurity] = 0

total_images = 0

def upload_detection_results():
    global impurity_dict
    global total_images
    global batch_id
    """
    Uploads detection results to Firebase under the given batch ID.
    """
    detections_ref = batch_ref.child("detections")

    for key, value in impurity_dict.items():
        current_value = detections_ref.child(key).get() or 0
        detections_ref.child(key).set(current_value + value)
...
def periodic_upload():
    with ThreadPoolExecutor(max_workers=1) as executor:
        while True:
            ...
            executor.submit(upload_detection_results)

# Start periodic upload in a daemon thread
            ...
            for i, result in enumerate(results):
                ...
                for box in result.boxes:
                    ...
                    if confidence_score >= self.confidence_threshold and class_score in allowed_classes:
                        ...
                        impurity = impurity_classes[class_score]
                        print(impurity)
                        impurity_dict[impurity] += 1
...
if __name__ == "__main__":
    ...
    def trigger_once():
        ...
        global detector
        global counter
        folder_path = "/home/cotseeds/Desktop/Cotseeds_Final/Code/images"
        image_extension = "*.bmp"
        ...
        try:
            while True:
                if stop_thread:
                    break
                image_files = []
                if not any(os.scandir('/home/cotseeds/Desktop/Cotseeds_Final/Code/images')):
                    ret = obj_cam_operation.Trigger_once()
                    pic_click_time = time.time()
                    if ret != 0:
                        strError = f"TriggerSoftware failed ret: {ToHexStr(ret)}"
                        print(strError)
                    while not any(os.scandir('/home/cotseeds/Desktop/Cotseeds_Final/Code/images')):
                        pass
                else:
